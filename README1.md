# Домашнее задание к занятию "`Практическое применение Docker" - `Первушин Дмитрий`

### Задача 1

Сделайте в своем GitHub пространстве fork репозитория.

1. Создайте файл ```Dockerfile.python``` на основе существующего ```Dockerfile```:

2. Используйте базовый образ ```python:3.12-slim```
