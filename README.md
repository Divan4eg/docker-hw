# Домашнее задание к занятию "`Оркестрация группой Docker контейнеров на примере Docker Compose`" - `Первушин Дмитрий`
...
Собрал образ с тэгом 1.0.0
```
docker build -t custom-nginx:1.0.0 .
```

К сожалению, на DockerHub загрузить не смог. Удалось зарегистрироваться, но дальше не открывается страница. Поэтому прикладываю сюда:
[Dockerfile](https://github.com/Divan4eg/docker-hw/blob/main/custom-nginx/Dockerfile)
...
1. Подключиться к стандартному потоку ввода/вывода/ошибок контейнера "custom-nginx-t2" можно командой:
```
docker attach custom-nginx-t2
```

2. Прикладываю скриншот:
![Task3](https://github.com/Divan4eg/docker-hw/blob/main/img/4.png)

3. Контейнер остановился, так как комбинация клавиш Ctrl+C посылает процессу, который выполняется в контейнере, сигнал SIGINT, что приводит к его остановке. Это видно в строке 1 вывода.

4. Перезапустить контейнер можно командой
```
docker restart custom-nginx-t2
```
![Task3](https://github.com/Divan4eg/docker-hw/blob/main/img/5.png)

5. Подключаемся к контейнеру командой
```
...
apt install nano
```
![Task3](https://github.com/Divan4eg/docker-hw/blob/main/img/6.png)

7. Редактируем файл командой
```
nano /etc/nginx/conf.d/default.conf
```
![Task3](https://github.com/Divan4eg/docker-hw/blob/main/img/7.png)

8. Прикладываю скриншот
![Task3](https://github.com/Divan4eg/docker-hw/blob/main/img/8.png)

10. Прикладываю скриншот
![Task3](https://github.com/Divan4eg/docker-hw/blob/main/img/9.png)  
Проблема возникла, так как теперь nginx внутри контейнера слушает порт 81 для входящих соединенй, а мы с локальной машины, заходя на порт 8080, внутрь контейнера стучимся на порт 80, а на нем уже никакой процесс не ждет входящих соединений.

11. Можно настроить iptables для перенаправления трафика с порта 80 на порт 81 внутри контейнера
```
docker exec -ti custom-nginx-t2 bash
iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 81
```

12. Удалить контейнер без остановки можно командой
```
